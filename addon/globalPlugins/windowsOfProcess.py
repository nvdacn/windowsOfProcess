...
def callback(hwnd, pid):
	...
	if winUser.isWindowVisible(hwnd) and winUser.isWindowEnabled(hwnd):
		...
		if processName not in epl and wid not in pl:
			pl.append(wid)
			hd[wid] = []
		if processName == "explorer.exe":
			if winUser.getWindowText(hwnd) and hwnd not in hd[wid]:
				hd[wid].append(hwnd)
			return True
		if wid in pl and hwnd not in hd[wid]:
			hd[wid].append(hwnd)
	return True
...
def nextWindow():
	...
	if len(hd[pid]) > 1:
		...
		if fg.windowHandle == hd[pid][index]:
			return nextWindow()
		if (
			winUser.isWindowVisible(hd[pid][index])
			and winUser.isWindowEnabled(hd[pid][index])
			and winUser.user32.SetForegroundWindow(hd[pid][index])
		):
			showWindow()
		else:
...
def nextProcessWindow(newPid):
	try:
		if hd[newPid][0]:
			if (
				winUser.isWindowVisible(hd[newPid][0])
				and winUser.isWindowEnabled(hd[newPid][0])
				and winUser.user32.SetForegroundWindow(hd[newPid][0])
			):
				showWindow()
				return True
			else:
